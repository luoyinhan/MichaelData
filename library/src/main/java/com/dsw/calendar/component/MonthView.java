...
public abstract class MonthView extends View {
    protected int NUM_COLUMNS = 7;//7列
    protected int NUM_ROWS = 6;// 6行
    ...
    private IleftandRightClick LeftandRightCallback = null;

    public void setLeftandRightCallback(IleftandRightClick leftandRightCallback) {
        LeftandRightCallback = leftandRightCallback;
    }

    public MonthView(Context context, AttributeSet attrs) {
        super(context, attrs);
        density = getResources().getDisplayMetrics().density;
        mScroller = new Scroller(context);//滑动器 状态
        ...
        setSelectDate(currYear, currMonth, currDay);//选择当前日期
        setLeftDate();//上个月 年月日
        setRightDate();//下个月 年月日
        createTheme();
        rowSize = theme == null ? 60 : theme.dateHeight();
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        setMeasuredDimension(widthSize, heightSize);//设置控件的大小
    }

    @Override
    protected void onDraw(Canvas canvas) {
        canvas.drawColor(theme.colorMonthView());//背景颜色
        if (smoothMode == 1) {//滑动模式
    ...
    private void drawDate(Canvas canvas, int year, int month, int startX, int startY) {
        ...
        NUM_ROWS = getMonthRowNumber(year, month);//有多少行
        columnSize = getWidth() * 1.0F / NUM_COLUMNS;
        rowSize = getHeight() * 1.0F / NUM_ROWS;
        daysString = new int[6][7];//日期数组
    ...
    protected abstract void drawBG(Canvas canvas, int column, int year, int month, int row, int day);
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        switch (eventCode) {
            ...
            case MotionEvent.ACTION_UP:
                ...
                if (upX - downX > 0 && Math.abs(upX - downX) > mTouchSlop * 6) {//左滑
                    if (smoothMode == 0) {
                        setLeftDate();
                        indexMonth--;
                        callbackScroll();
                ...
                } else if (upX - downX < 0 && Math.abs(upX - downX) > mTouchSlop * 6) {//右滑
                    if (smoothMode == 0) {
                        setRightDate();
                        indexMonth++;
                        callbackScroll();
                ...
                } else if (Math.abs(upX - downX) < 10 && Math.abs(upY - downY) < 6) {//点击事件
                    performClick();
    ...
    private void callbackScroll() {
        ...
        if (LeftandRightCallback != null) {
            LeftandRightCallback.action(selYear + time);
        }
    }
    ...
    private void doClickAction(int x, int y) {
        ...
        if (TextUtils.isEmpty(iscalendarInfo(selYear, selMonth, daysString[row][column]))) {//如果不可点击 则 屏蔽
            return;
        }
    ...
    private void setLeftDate() {
        ...
        if (month == 0) {//若果是1月份，则变成12月份
            ...
            month = month - 1;
            day = DateUtils.getMonthDays(year, month);//当年当月 有多少天
        } else {
            month = month - 1;
        }
    ...
    public interface IleftandRightClick {
        void action(String yearmonth);
    }
