...
public class CircleMonthView extends MonthView {
    ...
    protected void drawBG(Canvas canvas, int column, int row, int year, int month, int day) {
        if (day == selDay && month == selMonth) { //选中背景颜色
            ...
            float radius = columnSize < rowSize ? columnSize / 3 : rowSize / 3;
            paint.setColor(Color.parseColor("#ec3737"));//红色
    ...
    @Override
    protected void drawDecor(Canvas canvas, int column, int row, int year, int month, int day) {//添加描述
        if (calendarInfos != null && calendarInfos.size() > 0) {
            if (TextUtils.isEmpty(iscalendarInfo(year, month, day))) {//当日 是否可选
                return;
            }
...
//          canvas.drawCircle(circleX, circleY, theme.sizeDecor(), paint);
            if (day != selDay) {
                ...
                float cy = (startRecY + endRecY) / 2;
                float radius = columnSize < rowSize ? columnSize / 3 : rowSize / 3;
                paint.setColor(Color.parseColor("#06c371"));//绿色
                paint.setStyle(Paint.Style.FILL);
                canvas.drawCircle(cx, cy, radius, paint);
            }
        }
    ...
    @Override
    protected void drawText(Canvas canvas, int column, int row, int year, int month, int day) {//文本 写入
...
//                canvas.drawText(des, priceX, priceY, paint);//描述文字 屏蔽
            } else {//des为空的时候
...
//                canvas.drawText(des, priceX, priceY, paint);
    ...
    protected void createTheme() {
        theme = new DefaultDayTheme();//默认配置
    }
}
