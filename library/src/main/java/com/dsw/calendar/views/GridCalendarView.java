...
public class GridCalendarView extends LinearLayout implements View.OnClickListener {
    ...
    public GridCalendarView(Context context, AttributeSet attrs) {
        ...
        View view = LayoutInflater.from(context).inflate(R.layout.display_grid_date, null);//头部导航
        weekView = new WeekView(context, null);//星期控件
        gridMonthView = new GridMonthView(context, null);//日期列表
        ...
        gridMonthView.setMonthLisener(new MonthView.IMonthLisener() {//默认设置
            @Override
    ...
    public void setCalendarInfos(List<CalendarInfo> calendarInfos) {//设置 附加的状态
        gridMonthView.setCalendarInfos(calendarInfos);
    }

    public void setDayTheme(IDayTheme theme) {//设置日期控件样式
        gridMonthView.setTheme(theme);
    }

    public void setWeekTheme(IWeekTheme weekTheme) {//设置星期样式
